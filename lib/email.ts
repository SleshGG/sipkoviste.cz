...
export async function sendEmail(to: string, subject: string, html: string): Promise<string | null> {
  ...
  if (!apiKey?.trim()) {
    console.error('[Email] RESEND_API_KEY není nastaven – přidej do Vercel → Settings → Environment Variables')
    return null
  }
  try {
    ...
    console.log('[Email] Odesláno OK:', { to: to.slice(0, 3) + '***', subject, resendId: data?.id })
    return null
  } catch (err) {
...
export async function getUserEmail(userId: string): Promise<string | null> {
  ...
  if (!email) {
    console.warn('[Email] getUserEmail: e-mail pro userId', userId.slice(0, 8) + '...', 'nenalezen (auth.users)')
  }
  return email
}
