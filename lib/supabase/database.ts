...
import type { Product, ProductWithSeller, Profile, Message, MessageWithRelations, Review, ProductInsert, MessageInsert, ConfirmedSale, Notification } from './types'
...
/** Celkový počet viditelných neprodaných produktů (aktivní). */
...
/** Rezervované položky uživatele (produkty, které si rezervoval jako kupující). */
export async function getReservedItemsForUser(buyerId: string): Promise<Array<{
  id: string
  product_id: string
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      seller:profiles!products_seller_id_fkey(id, name, avatar_url)
    `)
  ...
  return (data ?? []).map((row: { id: string; name: string; brand: string; price: number; condition: string; image: string | null; reservation_expires_at: string | null; seller?: { id: string; name: string | null; avatar_url: string | null } }) => ({
    id: `reserved-${row.id}`,
    product_id: row.id,
    reservation_expires_at: row.reservation_expires_at,
    product: { id: row.id, name: row.name, brand: row.brand, price: row.price, condition: row.condition, image: row.image },
    seller: row.seller ?? { id: '', name: null, avatar_url: null },
  }))
}
...
export async function getNotifications(userId: string, limit = 50): Promise<(Notification & { product?: { id: string; name: string; image: string | null } | null })[]> {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('notifications')
    .select(`
      *,
      product:products(id, name, image, status)
    `)
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
    .limit(limit)
  ...
  return (data ?? []) as (Notification & { product?: { id: string; name: string; image: string | null; status?: string } | null })[]
}
...
/** Vrátí buyer_id pro prodaný produkt (pro zobrazení tlačítka Ohodnotit). */
export async function getSaleBuyerId(productId: string): Promise<string | null> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('confirmed_sales')
    .select('buyer_id')
