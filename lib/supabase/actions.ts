...
import { checkRateLimit, checkLoginBlock, recordFailedLogin, recordSuccessfulLogin, isSafeImageUrl } from '@/lib/security'
import {
  canUserRateProfile,
  getExistingReview,
...
import { getBuyIntentEmailHtml, getBuyRequestEmailHtml, getOfferEmailHtml, getOfferAcceptedEmailHtml, getCounterOfferEmailHtml } from '@/lib/email-templates'
...
export async function signUp(email: string, password: string, name: string) {
  const rlId = await getRateLimitId()
  if (!(await checkRateLimit('auth', rlId))) return { error: 'Příliš mnoho pokusů. Zkuste to za chvíli.' }
  ...
  if (data?.user?.id) {
    await auditLog(data.user.id, 'registration', { resource_id: data.user.id })
  }
  return { data }
}
...
export async function signIn(email: string, password: string) {
  const rlId = await getRateLimitId()
  if (await checkLoginBlock(rlId)) return { error: 'Příliš mnoho neúspěšných pokusů. Zkuste to za 15 minut.' }
  if (!(await checkRateLimit('auth', rlId))) return { error: 'Příliš mnoho pokusů. Zkuste to za chvíli.' }
  const err = validateAuthInput(email, password)
  if (err) return { error: err }
  ...
  if (error) {
    await recordFailedLogin(rlId)
    return { error: error.message }
  }

  await recordSuccessfulLogin(rlId)
  if (data?.user?.id) {
    await auditLog(data.user.id, 'login', { resource_id: data.user.id })
  }
...
export async function deleteAccountAction(password: string) {
  ...
  const uid = user.id
  await auditLog(uid, 'account_delete', { resource_id: uid })
...
export async function createProductAction(product: Omit<ProductInsert, 'seller_id'>) {
  const rlId = await getRateLimitId()
  if (!(await checkRateLimit('product', rlId))) return { error: 'Příliš mnoho akcí. Zkuste to za chvíli.' }
  const supabase = await createClient()
  ...
  await auditLog(user.id, 'product_create', {
    resource_id: data.id,
...
export async function updateProductAction(id: string, updates: Partial<ProductInsert>) {
  ...
  if (safeUpdates.price !== undefined && oldProduct && oldProduct.price !== data.price) {
    await auditLog(user.id, 'product_price_change', {
      resource_id: id,
      changed: { price: { from: oldProduct.price, to: data.price } },
    })
  }
...
export async function deleteProductAction(id: string) {
  ...
  await auditLog(user.id, 'product_delete', {
    resource_id: id,
...
export async function sendMessageAction(message: Omit<MessageInsert, 'sender_id'> & {
  ...
  const rlId = await getRateLimitId()
  if (!(await checkRateLimit('msg', rlId))) return { error: 'Příliš mnoho zpráv. Zkuste to za chvíli.' }
  const supabase = await createClient()
  ...
  await auditLog(user.id, 'message_send', {
    resource_id: data.id,
    receiver_id: message.receiver_id,
...
async function hasRecentBuyOrOfferEmail(
  ...
  const { data } = await supabase
    ...
    .in('message_type', ['buy', 'buy_request', 'offer'])
    .gte('created_at', since)
    .limit(1)
  return (data?.length ?? 0) > 0
}

/** Rezervovat produkt – kupující rezervuje, prodejce má 24h na potvrzení. */
export async function reserveProductAction(
  productId: string,
  receiverId: string,
  productName: string,
  sellerName: string
): Promise<{ error?: string; data?: { messageId: string } }> {
  const rlId = await getRateLimitId()
  if (!(await checkRateLimit('msg', rlId))) return { error: 'Příliš mnoho akcí. Zkuste to za chvíli.' }
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Musíte být přihlášeni.' }
  if (user.id === receiverId) return { error: 'Nemůžete rezervovat vlastní produkt.' }
  ...
  if (rpcError) {
    console.error('Error reserving product:', rpcError)
    return { error: rpcError.message || 'Nepodařilo se rezervovat.' }
  }

  const result = rpcResult as { ok?: boolean; error?: string; message_id?: string }
  if (!result?.ok) {
    return { error: result.error || 'Nepodařilo se rezervovat.' }
  }

  const alreadySent = await hasRecentBuyOrOfferEmail(supabase, user.id, receiverId, productId)
  if (!alreadySent) {
    const { data: sellerProfile } = await supabase.from('profiles').select('email_notifications').eq('id', receiverId).single()
    const sellerEmail = await getUserEmail(receiverId)
    if (sellerEmail && sellerProfile?.email_notifications !== false) {
      const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'
      const chatUrl = `${siteUrl}/messages?to=${user.id}&product=${productId}`
      const html = getBuyRequestEmailHtml(sellerName, productName, chatUrl)
      await sendEmail(sellerEmail, `Zájem o váš produkt: ${productName}`, html)
    }
  }
  ...
  return { data: { messageId: result.message_id ?? '' } }
}

/** Potvrdit prodej – prodejce potvrdí rezervaci. */
export async function confirmSaleAction(productId: string): Promise<{ error?: string }> {
  if (!UUID_REGEX.test(productId)) return { error: 'Neplatné ID.' }
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Musíte být přihlášeni.' }

  const { data: rpcResult, error: rpcError } = await supabase.rpc('confirm_sale', {
    p_product_id: productId,
  })

  if (rpcError) {
    console.error('Error confirming sale:', rpcError)
    return { error: rpcError.message || 'Nepodařilo se potvrdit prodej.' }
  }

  const result = rpcResult as { ok?: boolean; error?: string }
  if (!result?.ok) {
    return { error: result.error || 'Nepodařilo se potvrdit prodej.' }
  }
...
/** Vypršení rezervací – volá RPC expire_reservations. Použij při načtení produktu s vypršenou rezervací. */
export async function expireReservationsAction(): Promise<{ error?: string }> {
  const supabase = await createClient()
...
/** Zrušit rezervaci – prodejce zruší rezervaci. */
export async function cancelReservationAction(productId: string): Promise<{ error?: string }> {
  if (!UUID_REGEX.test(productId)) return { error: 'Neplatné ID.' }
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Musíte být přihlášeni.' }

  const { data: rpcResult, error: rpcError } = await supabase.rpc('cancel_reservation', {
    p_product_id: productId,
  })

  if (rpcError) {
    console.error('Error canceling reservation:', rpcError)
    return { error: rpcError.message || 'Nepodařilo se zrušit rezervaci.' }
  }

  const result = rpcResult as { ok?: boolean; error?: string }
  if (!result?.ok) {
    return { error: result.error || 'Nepodařilo se zrušit rezervaci.' }
  }
...
/** Koupit – LEGACY (okamžitý prodej). Zachováno pro zpětnou kompatibilitu, ale nyní se používá reserveProductAction. */
export async function sendBuyIntentAction(
  ...
  if (!alreadySent) {
    ...
    if (sellerEmail && sellerProfile?.email_notifications !== false) {
      ...
      console.warn('[Email] Koupit: e-mail prodejce nenalezen – zkontroluj SUPABASE_SERVICE_ROLE_KEY na Vercelu')
    }
...
export async function sendOfferAction(
  ...
  if (!alreadySent) {
    ...
    if (sellerEmail && sellerProfile?.email_notifications !== false) {
      ...
      console.warn('[Email] Nabídka: e-mail prodejce nenalezen – zkontroluj SUPABASE_SERVICE_ROLE_KEY na Vercelu')
    }
  }
...
export async function acceptOfferAction(messageId: string): Promise<{ error?: string }> {
  ...
  const { data: buyerProfile } = await supabase.from('profiles').select('email_notifications').eq('id', msg.sender_id).single()
  const offerRecipientEmail = await getUserEmail(msg.sender_id)
  if (offerRecipientEmail && buyerProfile?.email_notifications !== false && msg.product_id) {
...
export async function sendCounterOfferAction(
  ...
  const { data: buyerProfile } = await supabase.from('profiles').select('name, email_notifications').eq('id', origMsg.sender_id).single()
...
export async function submitReviewAction(params: {
  ...
  const rlId = await getRateLimitId()
  if (!(await checkRateLimit('review', rlId))) return { error: 'Příliš mnoho recenzí. Zkuste to za chvíli.' }
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) {
    return { error: 'Musíte být přihlášeni' }
  ...
  if (comment != null && comment.length > 2000) {
    return { error: 'Komentář je příliš dlouhý (max 2000 znaků).' }
  }
...
/** Vrátí jméno uživatele podle ID (pro zobrazení kupujícího u rezervace). */
export async function getProfileNameAction(userId: string): Promise<{ name: string | null; error?: string }> {
  if (!UUID_REGEX.test(userId)) return { name: null }
  const supabase = await createClient()
...
/** Vrátí upozornění pro přihlášeného uživatele. */
export async function getNotificationsAction(): Promise<{ notifications: Awaited<ReturnType<typeof getNotifications>>; error?: string }> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
...
/** Označí upozornění jako přečtené. */
export async function markNotificationAsReadAction(notificationId: string): Promise<{ error?: string }> {
  if (!UUID_REGEX.test(notificationId)) return { error: 'Neplatné ID upozornění.' }
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Musíte být přihlášeni.' }
  const { error } = await supabase
...
/** Označí všechna upozornění uživatele jako přečtená (při zobrazení stránky upozornění). */
...
export async function updateProfileAction(updates: { name?: string; avatar_url?: string; show_online_status?: boolean; email_notifications?: boolean }) {
  const supabase = await createClient()
  ...
  const toUpdate: { name?: string; avatar_url?: string; show_online_status?: boolean; email_notifications?: boolean } = { ...updates }
  ...
  if (updates.avatar_url !== undefined && updates.avatar_url !== null && updates.avatar_url !== '' && !isSafeImageUrl(updates.avatar_url)) {
    return { error: 'Neplatná URL avatara.' }
  }
  ...
  if (updates.name !== undefined) changed.name = toUpdate.name
  if (updates.avatar_url !== undefined) changed.avatar_url = '[updated]'
  if (updates.show_online_status !== undefined) changed.show_online_status = toUpdate.show_online_status
  if (updates.email_notifications !== undefined) changed.email_notifications = toUpdate.email_notifications
  if (Object.keys(changed).length > 0) {
    await auditLog(user.id, 'profile_update', {
      resource_id: user.id,
      changed,
    })
  }
