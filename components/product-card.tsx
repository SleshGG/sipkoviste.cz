...
function ProductCardInner({ product, index = 0, showFavorite, isFavorite, onToggleFavorite, isTogglingFavorite, favoriteCount = 0, priority, showViewCount, viewCount = 0, returnUrl }: ProductCardProps) {
  ...
  const productHref = returnUrl
    ...
    : `/product/${product.id}`
  const skipAnimation = priority && index < 2
  return (
    <motion.div
      initial={skipAnimation ? { opacity: 1 } : { opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: skipAnimation ? 0 : 0.25, delay: skipAnimation ? 0 : Math.min(index * 0.03, 0.3) }}
            ...
            {!('sold_at' in product && product.sold_at) && (
              <div className={`absolute bottom-1.5 right-1.5 sm:bottom-2 sm:right-2 flex items-center gap-0.5 rounded-lg border border-border bg-secondary pl-2.5 pr-3 py-1 ${favoriteCount <= 0 && !showFavorite ? 'justify-center pr-2.5' : ''}`}>
                {showFavorite && onToggleFavorite ? (
                  ...
                  <span className="flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center" title="Přihlaste se pro přidání do oblíbených">
                    <Heart className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" />
                  </span>
                )}
                {favoriteCount > 0 && (
