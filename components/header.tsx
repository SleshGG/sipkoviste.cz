...
export function Header({ variant = 'default' }: HeaderProps) {
  ...
  const fetchUnreadCount = async (userId: string) => {
    ...
    if (!msgRes.error) setUnreadMessagesCount(msgRes.count ?? 0)
    if (!notifRes.error) setUnreadNotificationsCount(notifRes.count ?? 0)
  }
  ...
  useEffect(() => {
    ...
    const loadProfileAndUnread = (userId: string) => {
      Promise.all([
        ...
        supabase.from('messages').select('*', { count: 'exact', head: true }).eq('receiver_id', userId).eq('is_read', false),
        supabase.from('notifications').select('*', { count: 'exact', head: true }).eq('user_id', userId).eq('is_read', false),
      ]).then(([profileRes, msgRes, notifRes]) => {
        if (!isMounted) return
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` }, () => fetchUnreadCount(user.id))
      .subscribe()
    return () => {
  ...
  return (
    <>
      ...
      className={cn(
        ...
        variant === 'overlay'
          ? 'fixed top-0 left-0 right-0 border-b border-border/50 bg-background/40 backdrop-blur-md supports-[backdrop-filter]:bg-background/30'
          : cn(
    ...
    >
      <div className="container mx-auto flex h-[72px] md:h-[80px] min-w-0 items-center justify-between gap-2 sm:gap-4 px-4">
        ...
        <Link href="/" className="flex h-full shrink-0 items-center" aria-label="Šipkoviště">
          <img
            ...
            height={50}
            fetchPriority="high"
            className="h-[32px] md:h-[44px] w-auto max-w-[200px] md:max-w-[240px] rounded-lg object-contain object-left"
          />
        </Link>
        ...
        <nav className="hidden lg:flex h-[80px] min-w-0 shrink-0 items-center gap-4 xl:gap-6 [&>a]:uppercase">
          <Link href="/sell">
            <Button size="default" className="gap-2 rounded-md uppercase">
              <Plus className="h-4 w-4" />
              Prodat
            </Button>
          </Link>
          <span className="h-[80px] w-0 border-l border-border shrink-0 self-stretch" aria-hidden />
          <Link href="/marketplace" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:no-underline transition-colors uppercase">
          ...
          {isLoggedIn && (
            <>
              <Link href="/messages" className="relative flex items-center gap-2 px-2 py-1.5 -mx-1 -my-0.5 rounded-md text-sm font-medium text-muted-foreground hover:text-foreground transition-colors uppercase">
                <span className="relative">
                  <MessageCircle className="h-5 w-5" />
                  {!isLoading && (unreadMessagesCount + unreadNotificationsCount) > 0 && (
                    <span className="absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-bold text-primary-foreground">
                      {(unreadMessagesCount + unreadNotificationsCount) > 99 ? '99+' : unreadMessagesCount + unreadNotificationsCount}
                    </span>
                  )}
                </span>
                Zprávy
          ...
          {isLoggedIn ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" sideOffset={1} className="w-56 min-w-56 max-w-56 border-t-0 rounded-t-none bg-menu-bg [&_a]:uppercase [&_button]:uppercase">
            ...
            <button
              type="button"
              className="flex items-center gap-3 self-stretch h-full min-h-0 w-56 -ml-4 xl:-ml-[25px] rounded-none pl-4 pr-3 py-2 text-left hover:bg-menu-bg focus:outline-none focus-visible:outline-none focus-visible:ring-0 transition-colors disabled:opacity-50 uppercase"
        ...
        <Sheet>
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-2 [&_a]:uppercase [&_button]:uppercase">
      ...
      {isMobile && !isProductDetail && variant !== 'overlay' && <div className="h-[72px] md:hidden shrink-0" aria-hidden />}
    </>
  )
}
