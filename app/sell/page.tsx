...
import { SteelDartsIcon, SoftDartsIcon, DartboardIcon, AccessoriesIcon } from '@/components/category-icons'
...
const categoryOptions = [
  { id: 'steel-darts', name: 'Ocelové šipky', icon: SteelDartsIcon, description: 'Ocelové hroty pro sisalové terče' },
  { id: 'soft-darts', name: 'Softové šipky', icon: SoftDartsIcon, description: 'Plastové hroty pro elektronické terče' },
  { id: 'dartboards', name: 'Terče', icon: DartboardIcon, description: 'Sisalové, elektronické a cvičné terče' },
  { id: 'accessories', name: 'Příslušenství', icon: AccessoriesIcon, description: 'Letky, násadky, pouzdra a další' },
]
...
export default function SellPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const uploadResult = await uploadProductImagesAction(formDataToSend)
      ...
      if (uploadedUrls.length === 0) {
        setError('Nepodařilo se nahrát žádný obrázek. Zkuste to znovu.')
        return
      }
                    ...
                    <div className="flex gap-3">
                      {formData.images[0] && (
                        <div className="relative h-16 w-16 rounded-lg overflow-hidden shrink-0 bg-secondary">
                          {!isSubmitting && formData.images[0].preview?.startsWith('blob:') ? (
                            <Image
