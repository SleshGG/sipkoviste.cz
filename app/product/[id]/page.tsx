...
import { getProductById, getProductFavoriteCounts, getSaleBuyerId, getProfile } from '@/lib/supabase/database'
import { defaultOgImage, defaultOgImageUrl } from '@/lib/site-config'
import { ProductPageClient } from './product-page-client'
import Link from 'next/link'
import { ArrowLeft } from 'lucide-react'

/** ISR – cache 60 s, revalidatePath při změně produktu */
export const revalidate = 60

async function getProduct(id: string) {
  try {
    const [product, buyerId] = await Promise.all([
      getProductById(id),
      getSaleBuyerId(id),
    ])
    
    if (product) {
      const reservedBy = (product as { reserved_by?: string | null }).reserved_by ?? null
      const reservedByName = reservedBy ? (await getProfile(reservedBy))?.name ?? null : null
      ...
      return {
        ...
        status: (product as { status?: string }).status ?? (product.sold_at ? 'sold' : 'active'),
        reserved_by: reservedBy,
        reservation_expires_at: (product as { reservation_expires_at?: string | null }).reservation_expires_at ?? null,
