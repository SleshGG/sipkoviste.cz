...
import { getFavoriteProductIdsAction, toggleFavoriteAction, getBulkAlreadyReviewedAction, submitReviewAction, deleteProductAction, confirmSaleAction, cancelReservationAction } from '@/lib/supabase/actions'
...
import { isUserOnline, formatMemberSince, formatLastSeen } from '@/lib/utils'
...
export function ProfileClient({ profile, products, soldItems, reviews, purchasedItems, reservedItems, favoriteCounts, productIdsCanDelete, isOwnProfile }: ProfileClientProps) {
  ...
  const handleConfirmSale = async (productId: string) => {
    setConfirmingProductId(productId)
    const err = await confirmSaleAction(productId)
  ...
  const handleCancelReservation = async (productId: string) => {
    setCancelingProductId(productId)
    const err = await cancelReservationAction(productId)
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <main className="container mx-auto px-4 sm:px-4 py-4 sm:py-6 pb-24 sm:pb-6">
            ...
            <div className="flex flex-row gap-4 sm:gap-6 items-start md:items-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2 text-sm text-muted-foreground">
                  {(() => {
                    const lastSeen = formatLastSeen(profile.show_online_status, profile.last_seen_at)
                    return lastSeen ? <div className="flex items-center gap-1.5"><span>{lastSeen}</span></div> : null
                  })()}
        ...
        {/* Banner: rezervace čekající na potvrzení */}
        {isOwnProfile && products.some((p) => (p as { status?: string }).status === 'reserved') && (
          <motion.div
            ...
            className="mb-4"
          >
            <Link href="/messages?tab=upozorneni">
              <Card className="border-primary/40 bg-primary/5 p-4 hover:bg-primary/10 transition-colors cursor-pointer">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm sm:text-base">
                      Máte {products.filter((p) => (p as { status?: string }).status === 'reserved').length} {products.filter((p) => (p as { status?: string }).status === 'reserved').length === 1 ? 'rezervaci' : 'rezervace'} čekající na potvrzení
                    </p>
                    <p className="text-xs sm:text-sm text-muted-foreground">Klikněte pro zobrazení upozornění</p>
                  </div>
                </div>
              </Card>
            </Link>
        ...
        >
          <h2 className="sr-only">Obsah profilu</h2>
          <Tabs defaultValue="nabidky" className="w-full">
            <TabsList className={`mb-4 grid w-full ${isOwnProfile ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2'} gap-2 p-2 h-auto min-h-0 bg-muted/50 rounded-xl border border-border`}>
              <TabsTrigger
                value="nabidky"
                className="flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-3 sm:py-2.5 min-h-[48px] sm:min-h-[44px] rounded-lg text-xs sm:text-sm font-medium data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm transition-all"
              >
                <Package className="h-4 w-4 shrink-0" />
                <span className="truncate min-w-0">{isOwnProfile ? 'Moje inzeráty' : 'Inzeráty'} ({products.length + soldItems.length})</span>
              </TabsTrigger>
              {isOwnProfile && (
                <>
                  <TabsTrigger
                  ...
                  >
                    ...
                    <span className="truncate min-w-0">Zakoupené ({purchasedItems.length})</span>
                  </TabsTrigger>
                  <TabsTrigger
                    value="rezervovane"
                    className="flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-3 sm:py-2.5 min-h-[48px] sm:min-h-[44px] rounded-lg text-xs sm:text-sm font-medium data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm transition-all"
                  >
                    ...
                    <span className="truncate min-w-0">Rezervované ({reservedItems.length})</span>
                  </TabsTrigger>
                </>
              )}
              <TabsTrigger
              ...
              >
                ...
                <span className="truncate min-w-0">Hodnocení ({reviews.length})</span>
              </TabsTrigger>
            </TabsList>
              ...
              {products.length > 0 || soldItems.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                  {products.map((product) => {
                    const p = product as ProductWithSeller & { status?: string; reserved_by?: string | null; reserved_by_name?: string | null }
                    ...
                    return isOwnProfile ? (
                      <Card key={product.id} className="border-border bg-card overflow-hidden flex flex-col p-0">
                        <div className="relative aspect-[3/4] bg-secondary">
                          ...
                          {isReserved && (
                            <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                              <Badge className="text-sm px-3 py-1 shrink-0 bg-amber-200 dark:bg-amber-800/50 text-amber-900 dark:text-amber-100 border-transparent shadow-md">
                                Rezervováno
                              </Badge>
                            </div>
                          )}
                        ...
                        <div className="p-3 flex flex-col flex-1">
                          ...
                          <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{product.brand} · {product.condition}</p>
                          {isReserved && p.reserved_by && (
                            <p className="text-xs text-muted-foreground mt-1">
                              {(p.reserved_by_name ?? 'Uživatel')} si chce koupit
                            </p>
                          )}
                            ...
                            {isReserved && p.reserved_by && (
                              <>
                                <Button
                                  ...
                                  onClick={() => handleConfirmSale(product.id)}
                                  disabled={!!confirmingProductId}
                                >
                                  {confirmingProductId === product.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3 w-3" />}
                                  Potvrdit prodej
                                </Button>
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-8 text-xs gap-1"
                                  onClick={() => handleCancelReservation(product.id)}
                                  disabled={!!cancelingProductId}
                                >
                                  {cancelingProductId === product.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <X className="h-3 w-3" />}
                                  Zrušit rezervaci
                                </Button>
                                <Button variant="outline" size="sm" className="h-8 text-xs gap-1" asChild>
                                  <Link href={`/messages?to=${p.reserved_by}&product=${product.id}`}>
                  ...
                  {soldItems.map((item) => (
                    <Card key={item.id} className="border-border bg-card overflow-hidden flex flex-col p-0">
                      ...
                      <div className="p-3 flex flex-col flex-1">
                        ...
                        <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{item.product?.brand} · {item.product?.condition}</p>
                        {item.buyer && (
                          <p className="text-xs text-muted-foreground mt-1">
            ...
            {isOwnProfile && (
              <TabsContent value="rezervovane" className="mt-0">
                {reservedItems.length > 0 ? (
                    ...
                    {reservedItems.map((item) => (
                      <Card key={item.id} className="border-border bg-card overflow-hidden flex flex-col p-0">
                        ...
                        <div className="p-3 flex flex-col flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Čeká na potvrzení prodejce
                          </p>
                          <p className="text-sm font-bold text-primary mt-2">
                            {(item.product?.price ?? 0).toLocaleString('cs-CZ')} Kč
                          </p>
                          <div className="flex flex-col gap-2 mt-3">
                            <Button variant="outline" size="sm" className="h-8 text-xs" asChild>
                              <Link href={`/product/${item.product_id}?from=${encodeURIComponent(`profile/${profile.id}`)}`}>Zobrazit inzerát</Link>
                            ...
                            <Button variant="outline" size="sm" className="h-8 text-xs gap-1" asChild>
                              <Link href={`/messages?to=${item.seller?.id}&product=${item.product_id}`}>
                                <MessageCircle className="h-3 w-3" />
                                Napsat prodejci
                              </Link>
                            </Button>
                  ...
                  <Card className="p-8 text-center border-dashed">
                    ...
                    <h3 className="text-lg font-semibold mb-2">Žádné rezervované zboží</h3>
                    <p className="text-muted-foreground text-sm">
                      Zatím nemáte žádné rezervované inzeráty. Po rezervaci produktu se zde zobrazí.
                    </p>
                  </Card>
                )}
              </TabsContent>
