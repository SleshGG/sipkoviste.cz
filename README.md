...
Spusť `scripts/init_all.sql` v Supabase SQL Editoru na prázdné databázi. Skript vytvoří všechny tabulky, funkce, triggery a RLS policy.

**Existující databáze:** Pokud už máš databázi z dřívějška, spusť `scripts/update_products_rls_for_reservation.sql` pro opravu viditelnosti rezervovaných produktů pro kupujícího.

Po spuštění vytvoř Storage bucket **product-images** (public) v Supabase Dashboard → Storage.
